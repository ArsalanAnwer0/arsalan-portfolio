  ...
  const toggleSubcategory = (subcategory: string) => {
    if (expandedSubcategories.includes(subcategory)) {
      setExpandedSubcategories(
        expandedSubcategories.filter((s) => s !== subcategory)
      );
    } else {
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-12 w-full max-w-7xl">
        {skillsData.map((category) => (
          <div key={category.name} className="space-y-4">
            ...
            >
              <h3 className="text-xl font-medium tracking-wide transition-colors duration-700">
                {category.name}
              </h3>
              <motion.div
                animate={{
                  rotate: expandedCategories.includes(category.name) ? 90 : 0,
                }}
                    ...
                    if ("skills" in item && Array.isArray(item.skills)) {
                      ...
                      return (
                        ...
                        >
                          ...
                          >
                            <h4 className="text-base font-medium transition-colors duration-700">
                              {subcategory.name}
                            </h4>
                            <motion.div
                              animate={{
                                rotate: expandedSubcategories.includes(
                                  subcategory.name
                                )
                                  ? 90
                                  : 0,
                            ...
                            {expandedSubcategories.includes(
                              ...
                              >
                                {subcategory.skills.map((skill) => (
                                  <div
                                    key={`${subcategory.name}-${skill.name}`}
                                    className={`flex justify-between items-center p-1 rounded transition-all duration-700 ${getHoverBg()}`}
                                  >
                                    <span
                                      className={`text-sm font-light transition-colors duration-700 ${getSkillTextColors()}`}
                                    >
                                      {skill.name}
                                    </span>
                                    {skill.level && (
                                      <span
                                        className={`text-xs transition-colors duration-700 ${getLevelColor(
                                          skill.level
                                        )}`}
                                      >
                                        {skill.level}
                      ...
                      return (
                        <div
                          ...
                          className={`flex justify-between items-center pl-2 p-1 rounded transition-all duration-700 ${getHoverBg()}`}
                        >
                          <span
                            className={`text-sm font-light transition-colors duration-700 ${getSkillTextColors()}`}
                          >
