      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-12 w-full max-w-7xl">
        {skillsData.map((category) => (
          <div key={category.name} className="space-y-4">
            {/* Category Header */}
            <div 
              className="flex items-center justify-between cursor-pointer group border-b border-white/20 pb-2 hover:border-white/40 transition-colors duration-300"
                    ...
                    if ('skills' in item && Array.isArray(item.skills)) {
                      ...
                      return (
                        <div key={`${category.name}-${subcategory.name}`} className="pl-2">
                          {/* Subcategory Header */}
                          <div 
                            className="flex items-center justify-between cursor-pointer group border-b border-white/10 pb-1 hover:border-white/30 transition-colors duration-300"
                            ...
                            {expandedSubcategories.includes(subcategory.name) && (
                              ...
                              >
                                {subcategory.skills.map((skill) => (
                                  <div key={`${subcategory.name}-${skill.name}`} className="flex justify-between items-center hover:bg-white/5 p-1 rounded transition-colors">
                      ...
                      return (
                        <div key={`${category.name}-${skill.name}`} className="flex justify-between items-center pl-2 hover:bg-white/5 p-1 rounded transition-colors">
                          <span className="text-sm font-light">{skill.name}</span>
                          {skill.level && (
                            <span className={`text-xs ${getLevelColor(skill.level)}`}>
                              {skill.level}
